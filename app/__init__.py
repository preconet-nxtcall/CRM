...
def create_app(config_class=Config):
    ...
    try:
        ...
        # Housing Job
        from app.services.housing_service import scheduled_housing_job
        if scheduler.get_job('housing_sync'):
             scheduler.remove_job('housing_sync')
             
        scheduler.add_job(
            id='housing_sync', 
            func=scheduled_housing_job, 
            args=[app], 
            trigger='interval', 
            minutes=10
        )
    ...
    app.register_blueprint(justdial_bp)

    from app.routes.housing import bp as housing_bp
    app.register_blueprint(housing_bp) # NEW
