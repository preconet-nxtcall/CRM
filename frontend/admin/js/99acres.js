...
class NinetyNineAcresManager {
    constructor() {
        this.statusContainer = document.getElementById("nna-status-container");
        this.connectForm = document.getElementById("nna-connect-form");
        this.connectBtn = document.getElementById("btn-nna-connect");
        this.syncBtn = document.getElementById("btn-nna-sync");

        this.emailDisplay = document.getElementById("nna-email-display");
        this.hostDisplay = document.getElementById("nna-host-display");
        this.lastSyncDisplay = document.getElementById("nna-last-sync-display");
    }
    ...
    async checkConnectionStatus() {
        try {
            const res = await auth.makeAuthenticatedRequest('/api/99acres/status');
            if (res && res.ok) {
            ...
            console.error("Error checking 99acres status", e);
            this.showDisconnected();
        }
    }
    ...
    async connect() {
        const email = document.getElementById("nna-email").value;
        const password = document.getElementById("nna-password").value;
        // const host = document.getElementById("nna-host").value || "imap.gmail.com"; // Hidden or default
        ...
        try {
            ...
            const response = await auth.makeAuthenticatedRequest('/api/99acres/connect', {
                method: 'POST',
            ...
            console.error("Error connecting 99acres", e);
            auth.showNotification("Failed to connect. Check credentials.", "error");
        } finally {
            if (this.connectBtn) {
                this.connectBtn.disabled = false;
                this.connectBtn.textContent = "Connect 99acres";
            }
        }
    }
    ...
    async sync() {
        try {
            ...
            const response = await auth.makeAuthenticatedRequest('/api/99acres/sync', {
                method: 'POST'
            });
    ...
    async disconnect() {
        if (confirm("Are you sure? This will stop lead sync.")) {
            try {
                const resp = await auth.makeAuthenticatedRequest('/api/99acres/disconnect', {
                    method: 'POST'
                });
                if (resp && resp.ok) {
                    ...
                    this.showDisconnected();
                    this.updateBadge(false);
                    document.getElementById("nna-email").value = "";
                    document.getElementById("nna-password").value = "";
                }
            } catch (e) {
    ...
    showConnected(settings) {
        ...
        const emailDisplay = document.getElementById("nna-email-display");
        if (emailDisplay) emailDisplay.textContent = settings.email_id;

        const lastSyncDisplay = document.getElementById("nna-last-sync-val");
        if (lastSyncDisplay) {
...
window.ninetyNineAcresManager = new NinetyNineAcresManager();
